...
def reformat_messages(parsed_content_list):
    ...
    for idx, element in enumerate(parsed_content_list):
        ...
        center_str = str([center_x, center_y])  # e.g., "[0.5, 0.3]"
        if element['type'] == 'text':
            screen_info += f'''<p id={idx} class="text" alt="{element['content']}" data-center="{center_str}"> </p>\n'''
        elif element['type'] == 'icon':
            screen_info += f'''<img id={idx} class="icon" alt="{element['content']}" data-center="{center_str}"> </img>\n'''
    return screen_info
