...
class OmniParserClient:
    ...
    def reformat_messages(self, response_json: dict):
        ...
        for idx, element in enumerate(response_json["parsed_content_list"]):
            ...
            center_str = f"[{center_x:.3f}, {center_y:.3f}]" 
            if element['type'] == 'text':
                screen_info += f'ID: {idx}, Text: {element["content"]}, Relative Position: {center_str}\n'
            elif element['type'] == 'icon':
                screen_info += f'ID: {idx}, Icon: {element["content"]}, Relative Position: {center_str}\n'
